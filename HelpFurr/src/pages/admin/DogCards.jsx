...
const DogCards = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`http://localhost:8080/dogs/approving/${props.dog._id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: "Approved"
        }),
  ...
  const deleteFormsAdoptedPet = async () => {
    setIsDeleting(true)
    try {
      const deleteResponses = await fetch(`http://localhost:8080/form/delete/many/${props.dog._id}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`http://localhost:8080/dogs/delete/${props.dog._id}`, {
        method: 'DELETE'
      })
    ...
    <div className='req-containter'>
      <div className='pet-view-card'>
        ...
        <div className='pet-card-details'>
          ...
          <p><b>Owner Email:</b> {props.dog.email}</p>
          <p><b>Owner Phone:</b> {props.dog.phone}</p>
          <p>
            <b>Condition:</b>
            <span>
              {truncateText(props.dog.condition, maxLength)}
              {props.dog.condition.length > maxLength && (
                <span onClick={() => setShowConditionPopup(!showConditionPopup)} className='read-more-btn'>
                  Read More
                </span>
              )}
            </span>
        ...
        {showConditionPopup && (
          <div className='popup'>
            ...
            <button onClick={() => setShowConditionPopup(!showConditionPopup)} className='close-btn'>
          ...
          <div className='popup'>
            <div className='popup-content'>
              ...
              <p>
                ...
                <a href={`mailto:${props.dog.email}`}>{props.dog.email}</a>{' '}
                or{' '}
                <a href={`tel:${props.dog.phone}`}>{props.dog.phone}</a>{' '}
